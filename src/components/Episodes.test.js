...
import { render, screen, getAllByTestId, queryAllByTestId } from '@testing-library/react'
...
test('displays shows after fetching show data', () => {
  const { rerender } = render(<Episodes episodes={[]} />)

  let episodes = screen.queryAllByTestId(/episodes/i)
  expect(episodes).toHaveLength(0)

  rerender(<Episodes episodes={episodeData} />)

  episodes = screen.getAllByTestId(/episodes/i)
